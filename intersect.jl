...
function get_eclass_constants(g::EGraph, c::EClass)
  constants = Dict{Union{Symbol,Int,Bool}, Vector{VecExpr}}()
  for node in c.nodes  
      v = v_head(node)
      c = get_constant(g, v)
      #typeof(c) == Symbol || (c = Symbol(string(c))) #TODO make this Union{Synbol,Expr,Int}
      haskey(constants, c) || (constants[c] = Vector{VecExpr}())
      push!(constants[c], node)
    end
  return constants
end
...
Go on until all equal expressions are found
"""
function eclass_intersect_many(gs::Vector{<:EGraph}, cs::Vector{<:EClass}=map(g -> g[g.root], gs), seen=Set{Vector{Id}}(), found=Dict{Vector{Id},Union{Vector{Union{Symbol,Expr,Int,Bool}},Nothing}}())
  ...
  common_constants = intersect(map(c -> Set{Union{Symbol,Int,Bool}}(collect(keys(c))), c_constants_to_nodes)...)
