...
function eclass_intersect(g₁::EGraph, g₂::EGraph, c₁::EClass=g₁[g₁.root], c₂::EClass=g₂[g₂.root], seen=Set{Tuple{Id, Id}}())
  ...
  #TODO define set of extracted programs
  ...
  for c in common_constants
    ...
    # if the nodes are constants, they must be equal
    length(children₁) > 0 || return true #TODO add constant to extracted programs and continue

    # else, for each child, recursively check if they have representations of that child that are equal
    all_equal = true
    #TODO make an array. for each i, store the found programs
    for i in eachindex(children₁)
      ...
      c_child₁ = g₁[child₁]
      c_child₂ = g₂[child₂]
      if !eclass_intersect(g₁, g₂, c_child₁, c_child₂, seen_child) #TODO change to != nothing
        all_equal = false
        break
      end
    end
    #TODO for each combination of found programs, perform a maketerm() and add to extracted programs

    if all_equal
      return true #TODO return all extracted programs
    end    
    
  end

  return false #TODO return Nothing
end
