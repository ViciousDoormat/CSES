...
function interpret_sygus(expr, state=nothing)
    if typeof(expr) != Expr 
        # if typeof(expr) == Bool && expr === true
        #     return :waar
        # elseif typeof(expr) == Bool && expr === false                                                   
        #     return :onwaar
        if expr == 0                                                  
            return Int(0)
        elseif expr == 1                                                
    ...
    if head != :call
        if head == :if
            return interpret_sygus(c[1], state) == :waar ? interpret_sygus(c[2], state) : interpret_sygus(c[3], state)
        end
        error(huilen2)
    end
    try
    MLStyle.@match c[1] begin
            ...
            :prefixof_cvc   => prefixof_cvc(interpret_sygus(c[2], state), interpret_sygus(c[3], state)) ? :waar : :onwaar
            :suffixof_cvc   => suffixof_cvc(interpret_sygus(c[2], state), interpret_sygus(c[3], state)) ? :waar : :onwaar
            :contains_cvc   => contains_cvc(interpret_sygus(c[2], state), interpret_sygus(c[3], state)) ? :waar : :onwaar
            
            :+              => interpret_sygus(c[2], state) + interpret_sygus(c[3], state)
            :-              => interpret_sygus(c[2], state) - interpret_sygus(c[3], state)
            :(==)           => interpret_sygus(c[2], state) == interpret_sygus(c[3], state) ? :waar : :onwaar
