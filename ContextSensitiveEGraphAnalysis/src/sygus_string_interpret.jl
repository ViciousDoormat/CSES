...
function interpret_sygus(expr, state)
    if typeof(expr) != Expr 
        return typeof(expr) == Symbol && haskey(state,expr) ? state[expr] : expr
    end
    ...
    if head != :call
        if head == :if
            return interpret_sygus(c[1], state) ? interpret_sygus(c[2], state) : interpret_sygus(c[3], state)
        end
        error(huilen2)
    end
